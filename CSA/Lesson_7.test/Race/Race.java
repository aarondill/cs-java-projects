...
/**
* The Race class's main method should print a greeting and then prompt the user 
* for the name of each DotRacer in the race. Instantiate each DotRacer object using
* the names provided by the user. Assign the DotRacers to an ArrayList. Next,
* prompt the user for the number of moves each DotRacer will make. Add a sorting
* method (selection or insertion) that will sort the DotRacer objects based on
* position from greatest to least. Print the racers in the order that they
* finished and print the names of the winner or winners.
*/
public class Race {
   ...
   public static void selectionSortDesc(List<DotRacer> list){
     for (int i = 0; i < list.size()-1; i++){
        int largestI = i;
        for (int j = i+1; j < list.size(); j++) if (list.get(j).getPosition() > list.get(largestI).getPosition()) largestI = j;
	...
	public static void main(String[] args) {
      ...
      DotRacer r;
		while( (r = makeRacer()) != null ) racers.add(r); // user is done when makeRacer returns null
      System.out.println();
   ...
   // Returns null when the user is done creating racers.
	...
	public static void race(int moves){
		for(int x = 1; x <= moves; x++) racers.forEach(DotRacer::move);
	}
	...
	public static void printRaceResults(){
      ...
      for(int i = 0; i < racers.size() && racers.get(i).getPosition() == first.getPosition(); i++) winnerNames.add(racers.get(i).getName());
      System.out.println();
