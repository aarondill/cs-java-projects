...
/**
 * Primary java class: ClassHealthQuiz
 *
 * <pre>
 * Author:  Aaron Dill
 * Date:    10/19/2023
 ...
 * </pre>
 *
 * Lab 3.3.2 –ClassHealthQuiz - Write a program that will assess your Computer
 ...
 * student's score. When the quiz is over, report to the student if their class
 * health is good or not. You can set the range. Caution: All questions and
 * output must be school appropriate.
 */
  ...
  public static void main(String... args) {
    try (Scanner scan = new Scanner(System.in)) {
      ...
      for (String question : questions) {
        ...
        String response = scan.next().intern();
        if (response != "1" && response != "0") {
          input_mismatch(String.format("Invalid response: '%s'. Valid responses only include 1 and 0.", response));
        }
        score += Byte.parseByte(response, 10);
      }
